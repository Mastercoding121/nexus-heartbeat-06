...
import { a as callMiddleware, n as defineLazyEventHandler, o as toMiddleware, r as toEventHandler, s as HTTPError, t as H3Core } from "./_libs/h3+rou3+srvx.mjs";
import { n as server_default$1, t as createServerEntry, v as NodeResponse } from "./_libs/@tanstack/react-start+[...].mjs";
...
var server_default = createServerEntry({ async fetch(request, env, ctx) {
	try {
		return await normalizeCatastrophicSsrResponse(await server_default$1.fetch(request, env, ctx));
	} catch (error) {
		console.error(error);
		return new Response(renderErrorPage(), {
			status: 500,
...
var _lazy_7TTTK5 = defineLazyEventHandler(() => import("./_chunks/ssr-renderer.mjs"));
var findRoute = /* @__PURE__ */ (() => {
	const $0 = {
		route: "/**",
		handler: multiHandler(toEventHandler(server_default), _lazy_7TTTK5)
	};
	return (m, p) => {
		if (p.charCodeAt(p.length - 1) === 47) p = p.slice(0, -1) || "/";
		let s = p.split("/");
