...
function writeLocalContacts(ownerId, contacts) {
  ...
  try {
    ...
    const contactsByOwner = Array.isArray(parsed)
      ? { [getContactOwnerKey(ownerId)]: parsed }
      : parsed;
    ...
    async (payload) => {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent("nexus:incoming-notification", {
            detail: {
              title: "New message",
...
export async function findMemberByNexusId(rawNexusId, currentUser) {
  ...
  if (parsed.status !== "exact_match")
    return { status: parsed.status, member: null, nexusId: parsed.value };
  ...
  try {
    ...
    if (member.is_active === false || member.isActive === false)
      return { status: "inactive", member: null, nexusId };
