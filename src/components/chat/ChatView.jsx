...
export default function ChatView({ chat, onBack, currentUserId = 'me', supportConversationId }) {
  ...
  const chatMembers = chat?.members && chat.members.length
    ? chat.members
    : chat?.peer_member_id
      ? [currentUserId, chat.peer_member_id].filter(Boolean)
      : [currentUserId, chat?.peer_id].filter(Boolean)

  const chatPeerMemberId = chatMembers.find((m) => m !== currentUserId)
    || chat?.peer_member_id
    || chat?.peer_id
  ...
  useEffect(() => {
    ...
    messages.forEach(async (msg) => {
      if (msg.encrypted && !decryptedMessages[msg.id]) {
        ...
        const decryptedFileName = msg.file_name
          ? await decryptMessage(chat.id, msg.file_name, true, e2eeContext)
          : undefined
  ...
  const handleSendMessage = async (messageData) => {
    ...
    if (encrypted) {
      setDecryptedMessages(prev => ({
        ...
        [`${newMessage.id}_fn`]: fileNameEncrypted ? (messageData.file_name || null) : undefined,
      }))
    }
            ...
            <MessageBubble
              ...
              message={{
                ...
                content: message.encrypted
                  ...
                  : message.content,
                file_name: message.encrypted
                  ? (decryptedMessages[`${message.id}_fn`] || message.file_name)
                  : message.file_name,
              }}
